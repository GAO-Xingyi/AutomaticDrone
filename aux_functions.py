...
def train_actor_critic(data_tuple_total, algorithm_cfg, agent, lr, lr_crit, epi_num):
    batch_size = algorithm_cfg.batch_size
    train_epoch_per_batch = algorithm_cfg.train_epoch_per_batch
    gamma = algorithm_cfg.gamma  # 折扣因子
    ...
    for i in range(num_batches):
        start_ind = i * batch_size
        end_ind = min(len(data_tuple_total), (i + 1) * batch_size)
        data_tuple = data_tuple_total[start_ind:end_ind]
        episode_len = len(data_tuple)

        # 初始化状态、动作、奖励等数组，确保数据类型一致
        curr_states = np.zeros((episode_len, agent.input_size, agent.input_size, 3), dtype=np.float32)
        next_states = np.zeros((episode_len, agent.input_size, agent.input_size, 3), dtype=np.float32)
        actions = np.zeros((episode_len, 1), dtype=np.int32)
        rewards = np.zeros((episode_len, 1), dtype=np.float32)
        dones = np.zeros((episode_len, 1), dtype=np.float32)

        # 填充数据并确保数据类型转换正确
        for ii, entry in enumerate(data_tuple):
            curr_state, action, next_state, reward, _, done = entry
            curr_states[ii] = np.array(curr_state, dtype=np.float32)
            next_states[ii] = np.array(next_state, dtype=np.float32)
            actions[ii] = np.array(action, dtype=np.int32)
            rewards[ii] = np.array(float(reward), dtype=np.float32)  # 确保为浮点数
            dones[ii] = np.array(float(not done), dtype=np.float32)  # 确保为浮点数

        # 针对每个批次进行训练
        for _ in range(train_epoch_per_batch):
            # 获取当前状态和下一状态的估计值
            V_s = agent.network_model.get_state_value(curr_states).astype(np.float32)
            V_s_next = agent.network_model.get_state_value(next_states).astype(np.float32)

            # 确保 rewards、V_s_next、dones 的数据类型一致
            TD_targets = rewards + gamma * V_s_next * (1.0 - dones)  # 使用 1.0 确保浮点运算
            advantages = TD_targets - V_s
            ...
            # agent.network_model.train_policy(
            #     curr_states, actions, TD_targets, advantages, lr, epi_num
            # )
            agent.network_model.train_policy(
                curr_states, actions, TD_targets, advantages,
                lr_actor=lr, lr_critic=lr_crit, iter=epi_num
            )
...
def policy_ActorCritic(curr_state, agent):
    action, p_a = agent.network_model.action_selection_with_prob(curr_state)
    action_type = 'Prob'
    return action[0], p_a, action_type

def policy_ActorCritic_infer(curr_state, agent):
    action = agent.network_model.action_selection(curr_state)
    action_type = 'Prob'
    return action[0], action_type
