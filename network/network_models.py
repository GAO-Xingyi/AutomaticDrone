...
# ActorCritic
#############################################################
# class initialize_network_DeepActorCritic():
#     def __init__(self, cfg, name, vehicle_name):
#         self.g = tf.Graph()
#         self.vehicle_name = vehicle_name
#         self.iter_baseline = 0
...
#
#             # Define losses
#             self.loss_actor_op = -tf.reduce_mean(tf.log(self.pi_a + 1e-10) * self.GAE)
#             self.loss_critic_op = 0.5 * tf.reduce_mean(tf.square(self.state_value - self.TD_target))
#             self.loss_entropy = -0.01 * tf.reduce_mean(tf.reduce_sum(self.pi * tf.log(self.pi + 1e-10), axis=1))
#
#             self.loss_op = self.loss_critic_op + self.loss_actor_op + self.loss_entropy
#
...
#         return baseline
#
#     def train_policy(self, xs, actions, TD_target, GAE, lr, iter):
#         self.iter_policy += 1
#         batch_size = xs.shape[0]
#         _, loss, loss_critic, loss_actor, loss_entropy = self.sess.run(
#             [self.train_op, self.loss_op, self.loss_critic_op, self.loss_actor_op, self.loss_entropy],
#             feed_dict={self.batch_size: xs.shape[0], self.learning_rate: lr,
#                        self.X1: xs,
#                        self.actions: actions,
#                        self.TD_target: TD_target,
...
#
#         # Log to tensorboard
#         self.log_to_tensorboard(tag='Loss_Total', group=self.vehicle_name, value=loss / batch_size,
#                                 index=self.iter_policy)
#         self.log_to_tensorboard(tag='Loss_Actor', group=self.vehicle_name, value=loss_actor / batch_size,
#                                 index=self.iter_policy)
#         self.log_to_tensorboard(tag='Loss_Critic', group=self.vehicle_name, value=loss_critic / batch_size,
#                                 index=self.iter_policy)
#         self.log_to_tensorboard(tag='Loss_Entropy', group=self.vehicle_name, value=loss_entropy / batch_size,
#                                 index=self.iter_policy)
#         self.log_to_tensorboard(tag='Learning Rate', group=self.vehicle_name, value=lr, index=self.iter_policy)
#
#     # def train_policy(self, xs, actions, TD_target, GAE, lr_actor, lr_critic, iter):
#     #     self.iter_policy += 1
#     #     batch_size = xs.shape[0]
#     #
#     #     # 使用单次 sess.run 同时更新 actor 和 critic
#     #     _, loss, loss_critic, loss_actor, loss_entropy = self.sess.run(
#     #         [self.train_op, self.loss_op, self.loss_critic_op, self.loss_actor_op, self.loss_entropy],
#     #         feed_dict={
...
#     #     self.log_to_tensorboard(tag='Loss_Entropy', group=self.vehicle_name, value=loss_entropy / batch_size,
#     #                             index=self.iter_policy)
#     #     self.log_to_tensorboard(tag='Learning Rate Actor', group=self.vehicle_name, value=lr_actor,
#     #                             index=self.iter_policy)
#     #     self.log_to_tensorboard(tag='Learning Rate Critic', group=self.vehicle_name, value=lr_critic,
#     #                             index=self.iter_policy)
#
#
#     def action_selection_with_prob(self, state, return_prob=False):
#         # 初始化动作和概率数组
#         action = np.zeros(dtype=int, shape=[state.shape[0], 1])
#         prob_action = np.zeros(dtype=float, shape=[state.shape[0], 1])
#
...
#
#     def save_network(self, save_path, episode=''):
#         save_path = f'{save_path}/{self.vehicle_name}/{self.vehicle_name}_{episode}'
#         self.saver.save(self.sess, save_path)
#         print('Model Saved:', save_path)
#
#     def load_network(self, load_path):
...
class initialize_network_DeepActorCritic():
    def __init__(self, cfg, name, vehicle_name):
        ...
        with self.g.as_default():
            ...
            # 定义占位符，包括两个学习率
            self.batch_size = tf.placeholder(tf.int32, shape=())
            self.learning_rate_actor = tf.placeholder(tf.float32, shape=(), name='learning_rate_actor')
            self.learning_rate_critic = tf.placeholder(tf.float32, shape=(), name='learning_rate_critic')
            self.X1 = tf.placeholder(tf.float32, [None, cfg.input_size, cfg.input_size, 3], name='States')
            self.X = tf.map_fn(lambda frame: tf.image.per_image_standardization(frame), self.X1)
            ...
            # 使用不同的学习率训练 Actor 和 Critic
            self.train_op = tf.train.AdamOptimizer(learning_rate=self.learning_rate_actor).minimize(self.loss_actor_op)
            self.critic_train_op = tf.train.AdamOptimizer(learning_rate=self.learning_rate_critic).minimize(self.loss_critic_op)
    ...
    def initialize_graphs_with_average(self, agent, agent_on_same_network):
        ...
        for i in range(len(values[name_agent])):
            val = [values[n][i] for n in agent_on_same_network]
            mean_val = np.average(val, axis=0)
            for name_agent in agent_on_same_network:
                var[name_agent][i].load(mean_val, agent[name_agent].network_model.sess)

    def prob_actions(self, xs):
        TD_target = np.zeros([xs.shape[0], 1], dtype=np.float32)
        GAE = np.zeros([xs.shape[0], 1], dtype=np.float32)
        actions = np.zeros([xs.shape[0], 1], dtype=int)
        return self.sess.run(self.pi, feed_dict={
            self.batch_size: xs.shape[0],
            self.X1: xs,
            self.actions: actions,
            self.TD_target: TD_target,
    ...
    def get_state_value(self, xs):
        baseline = self.sess.run(self.state_value, feed_dict={
            self.batch_size: xs.shape[0],
            self.X1: xs,
            self.actions: np.zeros([xs.shape[0], 1], dtype=int),
            self.TD_target: np.zeros([xs.shape[0], 1], dtype=np.float32),
            self.GAE: np.zeros([xs.shape[0], 1], dtype=np.float32)
        })
        return baseline
    ...
    def train_policy(self, xs, actions, TD_target, GAE, lr_actor, lr_critic, iter):
        ...
        # 使用单次 sess.run 同时更新 actor 和 critic
        _, _, loss, loss_critic, loss_actor, loss_entropy = self.sess.run(
            [self.train_op, self.critic_train_op, self.loss_op, self.loss_critic_op, self.loss_actor_op, self.loss_entropy],
            feed_dict={
                self.batch_size: xs.shape[0],
                self.learning_rate_actor: lr_actor,
                self.learning_rate_critic: lr_critic,
        ...
        # Log 到 TensorBoard
        self.log_to_tensorboard('Loss_Total', self.vehicle_name, loss / batch_size, self.iter_policy)
        self.log_to_tensorboard('Loss_Actor', self.vehicle_name, loss_actor / batch_size, self.iter_policy)
        self.log_to_tensorboard('Loss_Critic', self.vehicle_name, loss_critic / batch_size, self.iter_policy)
        self.log_to_tensorboard('Loss_Entropy', self.vehicle_name, loss_entropy / batch_size, self.iter_policy)
    ...
    def action_selection_with_prob(self, state):
        ...
        # 通过模型获取动作概率分布
        probs = self.sess.run(self.pi, feed_dict={
            self.batch_size: state.shape[0],  # 批量大小
            self.X1: state,  # 输入状态
            self.actions: action  # 初始化动作
        })
    ...
    def action_selection(self, state):
        ...
        probs = self.sess.run(self.pi,
                              feed_dict={self.batch_size: state.shape[0], self.learning_rate_actor: 1e-4,self.learning_rate_critic: 5e-5,
                                         self.X1: state,
                                         self.actions: action})
    ...
    def log_to_tensorboard(self, tag, group, value, index):
        summary = tf.Summary()
        summary.value.add(tag=f'{group}/{tag}', simple_value=value)
        self.stat_writer.add_summary(summary, index)
